...
def get_sse_bridge_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Failed to read from Assistant Core Settings ({e}), using environment variables and defaults")
    
    # Override with environment variables if available
    config["enabled"] = os.environ.get("SSE_BRIDGE_ENABLED", str(config["enabled"])).lower() in ('true', '1', 'yes', 'on')
    ...
    config["debug"] = os.environ.get("SSE_BRIDGE_DEBUG", os.environ.get("DEBUG", str(config["debug"]))).lower() in ('true', '1', 'yes', 'on')
...
def get_frappe_redis_config() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Redis config from Frappe: {redis_config['host']}:{redis_config['port']}/{redis_config['db']}")
        return redis_config
...
def get_fallback_redis_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Fallback Redis config: {redis_config['host']}:{redis_config['port']}/{redis_config['db']}")
